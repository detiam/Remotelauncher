    ...
    function(module, exports, __webpack_require__) {
                            ...
                            _.each(menuItem.subMenuItems, function(subMenuItem) {
                                ...
                                else
                                {
                                    ...
                                    if (subMenuItem.iconClass !== undefined && typeof subMenuItem.iconClass === 'string' ) {
                                        html += '<i class="Menuicon ' + subMenuItem.iconClass + '"></i> ';
                                    }
                                    html += '<span class="actionName"></span></a>';
                                    // end tag
                                    html += '</li>';
                                    subMenuItem.isSubmenuItem = true;
        ...
        function setupActionsEventListeners(_this) {
            ...
            _this.$menu.on(actionSelectEvent, function(evt) {
                ...
                if (!$menuItem || !$menuItem.length || $menuItem.is('.disabled')) {
                    return;
                }

                var index = $menuItem.data('menu-item');
        ...
        BootstrapMenu.prototype.open = function($openTarget, event) {
            ...
            var $menuItems = this.$menu.find('[data-menu-item]'),
            ...
            $menuItems.each(function(i, menuItem) {
                ...
                {
                  $menuItem.find('.Menuicon').attr('class','Menuicon ' + iconClass );
                };
                ...
                if (_.isFunction(menuItem.subMenuItems) && menuItem.subMenuItems(targetData)) {
                    ...
                    var subMenuItems = menuItem.subMenuItems(targetData),
                        $ul = $menuItem.find('ul.dropdown-menu'),
                        li = '',
                        $li;

                    $ul.empty();

                    if ( typeof subMenuItems === 'object' && subMenuItems.length ) {
                        _.each(subMenuItems, function(subMenuItem) {
                            ...
                            else
                            {
                                ...
                                if ( subMenuItem.iconClass !== undefined ) {
                                    li += '<i class="Menuicon ' + subMenuItem.iconClass + '"></i> ';
                                }
                                li += '<span class="actionName">' + subMenuItem.name + '</span></a>';
                                // end tag
                                li += '</li>';
                            }

                            $li = $(li);
                            if ( subMenuItem.onClick !== undefined )
                            {
                              $li.find('a').click(subMenuItem.onClick);
                            }
                            $li.appendTo($ul);
                        });
                    }
                ...
                if ( menuItem.isEnabled && menuItem.isEnabled(targetData) === false ) {
